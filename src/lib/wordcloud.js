// Ref: Wordcloud2 @ https://wordcloud2-js.timdream.org/
...
class WordCloud extends Component {
  ...
  render() {
    const { data, width, height, padding, font, fontSizeMapper, rotate, colors, color } = this.props;
    const wordCounts = data

    // clear old words
    select(this.wordCloud).selectAll('*').remove();

    const fillColor = (d, i) => i < colors.length ? colors[i] : colors[0]
